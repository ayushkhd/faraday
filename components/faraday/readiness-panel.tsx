...
export function ReadinessPanel({ preflight }: { preflight: Preflight | null }) {
  const signals = [
    ['OPENAI', Boolean(preflight?.openai.configured), preflight?.openai.configured ? 'Key configured' : 'Replay only'],
    ['GITHUB', Boolean(preflight?.github.reachable), preflight?.github.reachable ? 'Issue verified' : 'Live unavailable'],
