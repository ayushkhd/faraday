...
export function OutcomePanel({ state, source, latestWalls, busy, onReset, onRunAgain }: Props) {
  ...
  return (
    <section className={`outcome outcome-${state.verdict || 'error'}`} aria-live="assertive">
      <div className="outcome-head"><div><p className="section-index">03 / MACHINE VERDICT</p><h2>{state.verdict === 'breach' ? 'Boundary breached.' : state.verdict === 'contained' ? 'Attack contained.' : state.verdict === 'inconclusive' ? 'Evidence inconclusive.' : 'Run failed safely.'}</h2><p>{state.verdictReason || state.error}</p></div><span className="verdict-stamp">{state.verdict || 'error'}</span></div>
