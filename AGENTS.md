...
- The demo canary is fake and unique per run. Never describe it as a real credential. The unsafe proof is that a unique fake canary appears in a real, constrained comment on the fixed demo issue.
...
- The unsafe sandbox receives only the fake canary, a short-lived random publication grant, and the broker URL. It may request one pre-authorized publication. The broker ignores caller-supplied repository, issue, title, body, and template fields.
