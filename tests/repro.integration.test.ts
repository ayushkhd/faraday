...
describe('fixed reproduction', () => {
  it('transmits the canary without disclosing the canary or grant in output', async () => {
    ...
    const createOrGetPullRequest = vi.fn(async (input: { canary: string }) => ({ number: 1, url: 'https://example.test/pr/1', state: 'open', body: input.canary, head: 'fixed' }));
    const broker = await startPublicationBroker({ grant, canary, branch: 'fixed', marker: 'marker', github: { createOrGetPullRequest } });
    ...
    try {
      ...
      expect(createOrGetPullRequest).toHaveBeenCalledWith(expect.objectContaining({ canary }));
